...
public class FootballClub{
	...
	public String showInstallationsInfo(){
		...
		msg+="***************INSTALACIONES****************"+"/n"+
		"Hay "+dra+" jugadores del equipo "+this.teamA.getName()+" en el camerino A"+"/n"+
		"Hay "+drb+" jugadores del equipo "+this.teamB.getName()+" en el camerino B"+"/n"+
		"Hay "+office+" entrenadores en las oficinas"+"/n";
		return msg;
	}
			...
			for (int j=0;j<ORDER_OFFICE && !found;j++){
				if(this.office[i][j]!=null){
					if(this.office[i][j].getName().equalsIgnoreCase(name)){
						msg="El entrenador "+this.office[i][j].getName()+" se encuentra en la oficina "+i+", "+j;
					}
				}
			}
		}
			...
			for (int l=0;l<DR_COLUMNSA;l++){
				if(this.dressingRoomA[k][l]!=null){
					if(this.dressingRoomA[k][l].getName().equalsIgnoreCase(name)){
						msg="El jugador "+this.dressingRoomA[k][l].getName()+" se encuentra en el vestuario del equipo "+this.teamA.getName()+" en la ubicacion"+k+", "+l;
					}
				}
			}
		}
			...
			for (int n=0;n<ORDER_DRB;n++){
				if(this.dressingRoomB[m][n]!=null){
					if(this.dressingRoomB[m][n].getName().equalsIgnoreCase(name)){
						msg="El jugador "+this.dressingRoomB[m][n].getName()+" se encuentra en el vestuario del equipo "+this.teamB.getName()+" en la ubicacion"+m+", "+n;
					}
				}
			}
		}
	...
	public String updateEmployeesInfo(int yearsExperience, String name, double salary, int championships, String status, int attribute) {
		...
		for(int i=0; i<employees.size() && !found; i++) {
			if(employees.get(i) instanceof HeadCoach && employees.get(i).getIdentifier().equals(identifier)) {
				found = true;
				HeadCoach thisHeadCoach = (HeadCoach)employees.get(i);
				switch(attribute) {
					case 1:
					...
					case 2:
						...
						break;
					case 3:
						thisHeadCoach.setExperienceYears(experienceYears);
						break;
					case 4:
				...
				employees.get(i) = thisHeadCoach;
			}
		}
		if(found == false) {
			msg = "El empleado con ID "+identifier+" no es un entrenador principal o no esta registrado en el Club"+"\n";
		}
		return msg;
	}
	...
	public String addEmployee(int yearsExperience, String name, String id, double salary, int exPlayer, String expertise){
		...
		if(objSearch!=null){
			...
			AssistantCoach assistant = new AssistantCoach(yearsExperience, name, String id, salary, exPlayer, expertise);
			employee.add(player);
			addPl=true;
			message="The employee has been registered";
		}
	...
	public String updateEmployeesInfo(int yearsExperience,int attribute, String name, double salary, String status) {
		...
		boolean found = false;
		for(int i=0; i<employees.size() && !found; i++) {
			if(employees.get(i) instanceof AssistantCoach && employees.get(i).getIdentifier().equals(identifier)) {
				found = true;
				AssistantCoach thisAssistantCoach = (AssistantCoach)employees.get(i);
				switch(attribute) {
					case 1:
				...
				for(int j=0; j<3 && !found2; j++) {
					if(teamA.getAssistents()[j].getName().equalsIgnoreCase(name)) {
						teamA.setAssistens(thisAssistantCoach, j);
					}
					else if(teamB.getAssistents()[j].getName().equalsIgnoreCase(name)) {
						teamB.setAssistens(thisAssistantCoach, j)
					}
				}
				employees.get(i) = thisAssistantCoach;
			}
		}
		if(found == false) {
	...
	public String addEmployee(String name, String id, double salary, int dorsal, int rating, String position){
		...
		if(objSearch!=null){
			...
			Player player = new Player(String name, String id, double salary, int dorsal, String position);
	...
	public String updateEmployeesInfo(String name, double salary, String status, int dorsal, int goals, int rating, int attribute) {
		...
		for(int i=0; i<this.employee.size() && !found; i++) {
			if(employee.get(i) instanceof Players && employee.get(i).getName().equalsIgnoreCase(name)) {
				found = true;
				Players thisPlayer = (Players)employee.get(i);
				switch(attribute) {
					case 1:
				...
				if(teamA.findPlayer(name) != 30) {
					int index = teamA.findPlayer(identifier);
					teamA.getPlayers()[index] = thisPlayer;
				}
				else if(teamB.findPlayer(name) != 30) {
					int index = teamB.findPlayer(identifier);
					teamB.getPlayers()[index] = thisPlayer;
				}
				employees.get(i) = thisPlayer;
			}
		}
		if(found == false) {
	...
	public String dismiss(String name){
		...
		if(objSearch==null){
			...
			for(int j=0;j<this.teamA.getAssistents().length;j++){
				if(this.teamA.getAssistents()[j].getName().equalsIgnoreCase(name)){
					this.teamA.getAssistents()[j]=null;
					message+=" y eliminado del equipo al que pertenecia";
				}else if(this.teamB.getAssistents()[j].getName().equalsIgnoreCase(name)){
					this.teamB.getAssistents()[j]=null;
					message+=" y eliminado del equipo al que pertenecia";
				}
			}
			if(this.teamA.getHeadCoach().getName().equalsIgnoreCase(name)){
				this.teamA.getHeadCoach()=null;
				message+=" y eliminado del equipo al que pertenecia";
			}else if(this.teamB.getHeadCoach().getName().equalsIgnoreCase(name)){
				this.teamB.getHeadCoach()=null;
				message+=" y eliminado del equipo al que pertenecia";
			}
	...
	public String playerToTeam(String playerName, String teamName){
		...
		if(this.teamA.getName().equalsIgnoreCase(teamName)){
			for (int i=0;i<this.teamA.getPlayers().length;i++){
				if(this.teamA.getPlayers()[i]!=null && this.teamA.getPlayers()[i].equalsIgnoreCase(playerName)){
					msg="El jugador ya existe en el equipo";
				}else if(this.teamA.getPlayers()[i]!=null){
					if(findEmployee(playerName)){
			...
			for (int k=0;k<this.teamB.getPlayers().length;k++){
				if(this.teamB.getPlayers()[k]!=null && this.teamB.getPlayers()[k].equalsIgnoreCase(playerName)){
					msg="El jugador ya existe en el equipo";
	...
	public String assistantToTeam(String assistantName, String teamName){
		...
		boolean added=false;
		if(this.teamA.getName().equalsIgnoreCase(teamName)){
			for (int i=0;i<this.teamA.getAssistents().length && !added;i++){
				if(this.teamA.getAssistents()[i]!=null && this.teamA.getAssistents()[i].equalsIgnoreCase(assistantName)){
					msg="El Asistente ya existe en el equipo";
					added=true;
				}else if(this.teamA.getAssistents()[i]!=null){
					if(findEmployee(assistantName)){
						this.teamA.getAssistents()[i]=(AssistantCoach)findEmployee(assistantName);
			...
			for (int k=0;k<this.teamB.getAssistents().length && !added;k++){
				if(this.teamB.getAssistents()[k]!=null && this.teamB.getAssistents()[k].equalsIgnoreCase(assistantName)){
					msg="El Asistente ya existe en el equipo";
				}else if(this.teamB.getAssistents()[k]!=null){
					if(findEmployee(assistantName)){
						this.teamB.getAssistents()[k]=(AssistantCoach)findEmployee(assistantName);
						msg="Asistente agregado exitosamente"
					}else{
						msg="El asistente no existe";
					}
	...
	public String playerToDressingRoom(String teamName, String playerName){
		...
		if(newplayer==null){
			...
			for (int i=0;i<25 && !found;i++){
				if(this.teamA.getPlayers()[i].getName().equalsIgnoreCase(playerName)){
					found=true;
					for(int i=0;i<DR_ROWA && !added;i++){
						for(int j=0;j<DR_COLUMNSA && !added;j++){
							if(i==0 && j==0 && dressingRoomA[i][j]==0){
				...
				} else if(this.teamB.getPlayers()[i].getName().equalsIgnoreCase(playerName)){
					found=true;
	...
	public String showTeamsInfo(){
		...
		String msg+= this.teamB.showTeamInfo();
		return msg;
	}
	...
	public String showClubInfo(){
		...
		"Nombre: "+this.name+"/n"+
		"Nit: "+this.nit+"/n"+
		"Fecha de fundacion: "+this.foundationDate+"/n"+
		"Cantidad de empleados: "+this.employee.size()+"/n";
		if (this.teamA!=null){
			teamsAmount++;
		} 
		if (this.teamB!=null){
		...
		msg += "Cantidad de equipos: "+teamsAmount+"/n"+
		"Equipo A: "+this.teamA.getName()+"/n"+
		"Equipo B: "+this.teamB.getName()+"/n";
		return msg;
	}
}	