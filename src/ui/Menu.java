...
public class Menu{
	...
	public String readAddEmployee(){
		...
		if(choice==1){
			...
			String id=sc.nextLine(); 
			msg=this.club.addEmployee(name, id, salary, dorsal, int rating, String position);
		}else if(choice == 3){
			...
			msg=this.club.addEmployee( yearsExperience1, name, id, salary, exPlayer, String expertise)
		}else{
			msg="Opcion invalida";
		}
		return msg;
	...
	public String readUpdateTeam(){
		...
		if(choice==1){
			...
			System.out.println("Ingrese la tactica de la alineacion (Posesion, Contraataque, Presion alta, Por defecto)");
			String tactic=sc.nextLine(); 
			System.out.println("Ingrese la formacion del equipo SIN ESPACIOS (ej:4-4-2, 4-1-2-1-2-1)");
			System.out.println("Recuerde que debe tener 10 jugadores en la formacion, 3 lineas de jugadores como MINIMO y 6 como MAXIMO");
	...
	private String readUpdatePlayerInfo(){
		...
		switch(attribute){
			case 1:
				...
				sc.nextLine(); 
				msg=this.club.updateEmployeesInfo(String name, double salary, String status, int dorsal, int goals, int rating, int attribute);
				break;
			case 2:
	...
	public String readEmployeeLocation(){
		...
		String name = sc.nextLine();
		String msg = employeeLocation(name);
		return msg;
	}
